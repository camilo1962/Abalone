...
public class Movement : MonoBehaviour
{
  ...
  {
    Debug.Log("Comprobando si puede empujar");
    List<Node> nodeList = new List<Node>();
    foreach (GameObject tile in InputScript.selectedTiles)
    {
      nodeList.Add(abalone.getNode(tile));
    ...
    {
      consoleManager.sendMessageToConsole("Tratando de empujar el espacio vacío. Movimiento no válido");
      return false;
    }
    ...
    while (keepSearching)
    {
      if (next.getColor() == color)
      {
        keepSearching = false;
        canPush = false;
        consoleManager.sendMessageToConsole("Intentó empujar, pero chocó contra una de sus propias piezas.");
      }
      else if (next.getColor() == opponentColor)
      {
        ...
        if (checkIfNodeIsNullByDirection(direction, next))
        {
          ...
          if (pushPower > numberToPush)
          {
            ...
            consoleManager.sendMessageToConsole("Empujar una pieza fuera del tablero");
            consoleManager.sendMessageToConsole("Tratando de empujar " + numberToPush);
      ...
      else if (next.getColor() == BoardColor.EMPTY)
      {
        ...
        {
          consoleManager.sendMessageToConsole("Estás intentando empujar más fichas de las que puedes.");
        }
      }
    }
  ...
  public Node getHeadOfStackByDirection(Direction direction, List<GameObject> selectedList = null, List<Node> inputNodeList = null)
  {
    ...
    Node toReturn = new Node("Error al encontrar el encabezado del nodo", BoardColor.EMPTY, false, 0, 0);
  ...
  public void validateMovement(Direction direction)
  {
    ...
    {
      consoleManager.sendMessageToConsole("Intenté moverme sin ningún mosaico seleccionado. Movimiento no válido.");
      return;
    }
    //Check to make sure Tiles selected are actually in a moveable position
    consoleManager.sendMessageToConsole("Intentando moverse " + direction);
    if (!checkIfTilesAreInLine())
    {
      consoleManager.sendMessageToConsole("Los mosaicos seleccionados no están en una columna. Movimiento no válido.");
      return;
    }
    ...
    if (checkIfSideStep(direction))
    {
      ...
      {
        consoleManager.sendMessageToConsole("Paso lateral hacia un mosaico vacío. Ficha en movimiento");
        move(direction);
      ...
      {
        consoleManager.sendMessageToConsole("Intentó dar un paso lateral hacia una posición que no estuviera vacía. Movimiento no válido.");
        return;
      }
    }
    else
